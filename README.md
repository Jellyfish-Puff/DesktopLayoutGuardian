...
这是一个面向 Windows 11 单显示器切换场景的桌面图标布局管理器。目前版本为 0.6：重新设计的清晰 Fluent 风格界面预览。
...
- 可暂停自动恢复而不影响手动保存、恢复和撤销；
- 提供快速、标准、稳健三档检测策略，UU 超级屏可选稳健模式；
...
- 概览页集中展示当前方案、自动恢复状态、主要操作和已保存方案预览；
- 在“显示方案”页查看预览、重命名、恢复或安全删除方案；
- 再次保存同一方案前自动备份旧版本；
- 在“恢复记录”页查看并临时恢复覆盖保存前的旧布局；
- 可为每个方案保留 5、10、20 或 50 份历史版本；
- 手动恢复已保存布局；
- 切换到保存过的显示环境后自动恢复；
- 未保存过的显示环境不会移动图标；
...
- 恢复成功通知可选，失败始终提醒。
- 设置页支持清理超出数量的历史、无引用预览和诊断日志；
- 全部当前方案可安全移入 `deleted` 目录后重新开始。

当前版本只支持恰好一个活动显示器。刷新率不参与布局匹配；显示器身份、分辨率、缩放和方向参与匹配。

0.6 版本完全兼容此前已经保存的显示方案和设置，无需重新整理桌面；旧方案再次保存后会自动生成预览。
